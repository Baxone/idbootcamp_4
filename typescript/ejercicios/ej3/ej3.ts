...
function calcularLetraDni(pDni: number): string {

    if (pDni <= 0 || pDni >= 99999999) {
        return "dni no valido";
    }

    let posicion: number = pDni % 23;
    const arrayLetras: string[] = ['T', 'R', 'W', 'A', 'G', 'M', 'Y', 'F', 'P', 'D', 'X', 'B', 'N', 'J', 'Z',
        'S', 'Q', 'V', 'H', 'L', 'C', 'K', 'E', 'T']

    return arrayLetras[posicion];
}


console.log(calcularLetraDni(45678923))